...
export default function ConsultantTumorBoardsPage() {
  // Simulate the current logged-in consultant
  const currentConsultantId = "doc1" // Dr. Emily White
  const currentConsultantName = "Dr. Emily White"
  const currentConsultantType = "doctor" // Only doctors can access tumor boards

  // Dummy data for demonstration, filtered for tumor boards involving the current consultant
  ...
  const doctorsTumorBoards = allTumorBoards.filter(
    (board) =>
      board.primaryDoctorId === currentConsultantId || board.participatingDoctorIds.includes(currentConsultantId),
  )
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Tumor Boards Involving {currentConsultantName}</CardTitle>
